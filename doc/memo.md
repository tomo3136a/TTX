...
TTX を制御するための設定ファイル。
**_C11H17N3O8_** は、ふぐ毒で知られる テトロドトキシン (tetrodotoxin, TTX) の化学式。神経毒で
...
プラグインを有効・無効を制御したいプラグインには、TTXBind 関数内に **_TTXIgnore()_** 関数を追加する。
...
**_buf_** および **_buf_sz_** は、下記以外で値を変更しない。( **_buf_** の箇所は任意の名前)
...
複数のビルド環境を管理するのは出来ないので、 cmake だけにする。
ビルドに使う環境は Visual Studio の最新版を使用する。
