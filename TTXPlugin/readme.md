...
※拡張プラグインを無効化は、 **「ttx.txt に対応した TTX」**  のみ可能
...
1. プラグインをビルドします。
2. Tera Term 実行環境 (ttermpro.exeがあるフォルダ) にビルドした TTXOnOff.dll をコピーします。
...
本機能の制限として **「TTXOnOff に対応した TTX」** のみが制御対象となります。

TTX を **「TTXOnOff に対応した TTX」** にするには、各 TTX ソースコードに TTXIgnore() 関数追加と TTXBind() 関数に呼び出し文を追加します。
